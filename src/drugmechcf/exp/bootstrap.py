"""
Copied from scipy.stats._resampling.py
to add support for stratified bootstrap.
"""
...
from scipy.stats._axis_nan_policy import _broadcast_concatenate, _broadcast_arrays
from scipy.stats._warnings_errors import DegenerateDataWarning

import pandas as pd

__all__ = ['stratified_bootstrap', 'monte_carlo_test', 'permutation_test']
...
def stratified_bootstrap_resample(sample, groups, n_resamples=None, random_state=None):
    """Stratified Bootstrap resample the sample, using group-ids in `groups`."""

    resamples = []

    df = pd.DataFrame(dict(data=sample, group=groups))

    for gid, vdf in df.groupby(by="group"):
        arr = vdf["data"].values
        ...
        resamples.append(arr_resampled)

    return np.hstack(resamples)
...
def stratified_bootstrap(data, data_groups, statistic, *, n_resamples=9999, batch=None,
              vectorized=None, paired=False, axis=0, confidence_level=0.95,
              alternative='two-sided', method='BCa', bootstrap_result=None,
              random_state=None):
    r"""
    Same as `scipy.stats.bootstrap()`, except data has groups, with group-ids in the extra arg `data_groups`.
    Does stratified resampling using the group-ids.

        data_groups: a group-id for each sample in each dataset in data[...]
            Same shape as `data`, typically a seq of one vector of scalars.
    ...
    for k in range(0, n_resamples, batch_nominal):
        ...
        # Generate resamples
        resampled_data = []
        for sample, groups in zip(data, data_groups):
            resample = stratified_bootstrap_resample(sample, groups, n_resamples=batch_actual,
